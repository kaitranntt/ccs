...
function ensureWritableDirectory(downloadPath) {
  ...
  const containingRoot = findContainingRoot(resolvedPath, safeRoots);
  if (!containingRoot) {
    throw new Error(
      'downloadPath must be inside the browser session download directory or a CCS_BROWSER_DOWNLOAD_ROOTS entry'
    );
  }
  assertNoSensitiveLocalPathSegments(resolvedPath, containingRoot, 'downloadPath');

  const ancestorPath = fs.realpathSync(getNearestExistingAncestor(resolvedPath));
  if (!isPathInsideRoot(ancestorPath, containingRoot)) {
